...
class ThisREPO:
    ...
    def clean_old_zip(self):
        old_zip_path = self.config.zip_path
        if not old_zip_path.exists():
            print("未检测到旧的压缩文件，跳过清理。")
            return
        ...
        try:
            ...
            raise Exception(f"错误: 无法删除旧文件 {old_zip_path}。错误信息: {e}")
        print(f"已删除已存在的旧文件: {old_zip_path}\n")

    def tag(self):
        print("对 ThisREPO 目录执行 git tag")
        # 如果标签已存在，先在本地删除它再重新创建
        p = self._run_command(["git", "tag", "-d", self.config.repo_tag_name], check=False)
        ...
        self._run_command(
            [
                "git",
                "tag",
                self.config.repo_tag_name,
                "-a",
                "-m",
                self.config.repo_commit_message,
            ]
        )
    ...
    def push_main(self):
        ...
        self._run_command(["git", "push", "--force-with-lease", self.config.repo_remote_name, "main"])
